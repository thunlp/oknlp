...
class TestTasks(unittest.TestCase):
    def test_ner(self):
        ...
        for ner in ink.ner.get_all_ner():
            results = ner(sents)
    ...
    def test_cws(self):
        sents = ["清华大学自然语言处理与社会人文计算实验室"]
        for cws in ink.cws.get_all_cws():
            results = cws(sents)
            self.assertEqual(len(sents), len(results))
            for sent, result in zip(sents, results):
                sent_r = ''.join(result)
    ...
    def test_pos_tagging(self):
        sents = ["清华大学自然语言处理与社会人文计算实验室"]
        for pos_tagging in ink.postagging.get_all_pos_tagging():
    ...
    def test_typing(self):
        ...
        for typing in ink.typing.get_all_typing():
